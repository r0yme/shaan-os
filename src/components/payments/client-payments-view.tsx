...
import { AlertCircle, Banknote, Download, Paperclip } from "lucide-react";
...
import { recordClientPaymentAction } from "@/app/c/payments/actions";
import type { ActionResult } from "@/lib/action-result";

export interface ClientSerializedPayment {
  id: string;
  amountCents: number;
  method: string;
  paidAt: string;
...
function linkedLabel(payment: ClientSerializedPayment): string {
...
export function ClientPaymentsView({
  ...
  payments: ClientSerializedPayment[];
  totalPaidCents: number;
  clientName: string;
  invoiceOptions: LinkOption[];
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const result: ActionResult = await recordClientPaymentAction(formData);
    setLoading(false);
  ...
  return (
    <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground">
              Link a payment to one of your invoices, projects or tasks, and upload proof of
              payment (bank slip, screenshot, receipt).
            </p>
          </CardContent>
        </Card>
      </div>

      <form onSubmit={onSubmit} className="space-y-4 rounded-lg border border-border bg-card p-4 sm:p-5">
        {error && (
          <div
            role="alert"
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="space-y-2">
            <Label htmlFor="pay-task">Task</Label>
            <Select
              id="pay-task"
              value={taskId}
              onChange={(e) => {
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Wire number / transaction ID"
            />
          </div>
          <div className="space-y-2">
      ...
      <Card>
        ...
        <CardContent>
          <DataTable<ClientSerializedPayment>
            columns={[
              {
                key: "amount",
                header: "Amount",
